...
6. **Thématisation & Dark Mode (Vue Web)** : Toute la colorimétrie de l'application web doit strictement reposer sur les variables CSS sémantiques de PrimeVue (ex: `var(--p-surface-0)`) configurées au sein du `tailwind.config.js`. Ne jamais utiliser de modificateurs Tailwind du type `dark:bg-surface-900` sur les composants individuels, car les variables de PrimeVue s'inversent de façon autonome en mode sombre, ce qui génère des incohérences visuelles de type "Texte clair sur fond clair" si on force les deux en même temps. Un simple `bg-surface-0` suffit pour le jour et la nuit.
