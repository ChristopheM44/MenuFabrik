...
exports.analyzeRecipeWithGemini = void 0;
const admin = require("firebase-admin");
const https_1 = require("firebase-functions/v2/https");
const genai_1 = require("@google/genai");
admin.initializeApp();
exports.analyzeRecipeWithGemini = (0, https_1.onCall)({
    cors: true // Required for web apps
}, async (request) => {
    // Verifying that the user is authenticated.
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Vous devez être connecté pour utiliser l'IA.");
    }
    ...
    if (!instructions && !sourceUrl) {
        throw new https_1.HttpsError("invalid-argument", "Veuillez fournir des instructions ou une URL à analyser.");
    }
    try {
        ...
        const gGenAI = new genai_1.GoogleGenAI({ apiKey: apiKey });
        let promptText = "";
        if (sourceUrl) {
...
Note : Le tableau "allergens" contient les noms des allergènes détectés en chaîne de caractères. Si aucun allergène, renvoie []. L'unité de l'ingrédient peut être vide, g, ml, cuillères...
      `;
        ...
        throw new https_1.HttpsError("internal", "Erreur lors de l'analyse IA: " + error.message);
    }
});
