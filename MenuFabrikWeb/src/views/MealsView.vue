...
<template>
    <div class="w-full max-w-6xl mx-auto p-4 md:p-6 pb-32 min-h-screen animate-fadein">
        ...
        <div v-else class="space-y-12 relative">
            ...
            <div v-if="mealsByDate.length === 0" class="flex flex-col items-center justify-center p-12 bg-surface-container-low rounded-2xl border border-dashed border-outline-variant">
                <i class="pi pi-calendar-times text-4xl text-on-surface-variant mb-3"></i>
                <h3 class="text-lg font-semibold text-on-surface">Aucun repas prévu</h3>
                <p class="text-on-surface-variant text-center mb-6">Utilisez l'assistant pour planifier vos jours puis générez vos plats !</p>
                <button @click="showPlanDialog = true" class="px-6 h-12 rounded-full bg-primary text-on-primary font-bold text-sm shadow-lg hover:opacity-90 transition-all flex items-center gap-2">
            ...
            <div v-for="day in mealsByDate" :key="day.dateKey" class="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-6 md:gap-8 group">
                <!-- Sticky Sidebar -->
                <aside class="md:sticky md:top-24 h-fit flex flex-row md:flex-col items-center md:items-start justify-between md:justify-start gap-4 border-b border-outline-variant md:border-none pb-3 md:pb-0">
                    <div class="flex flex-row md:flex-col items-baseline md:items-start gap-2 md:gap-0">
                        <span class="text-2xl md:text-4xl font-extrabold text-on-surface md:group-hover:text-primary transition-colors">{{ day.label.split(' ')[0] }}</span>
                        <span class="text-on-surface-variant font-bold tracking-widest uppercase text-xs md:text-sm md:mt-1">
                    ...
                    <div class="flex gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                        <button @click="toggleAddMealMenu($event, day.dateKey)" class="w-8 h-8 md:w-10 md:h-10 rounded-full border border-outline-variant flex items-center justify-center hover:bg-primary hover:text-on-primary transition-all text-on-surface-variant bg-surface-container-low md:bg-transparent">
                            <i class="pi pi-plus text-sm md:text-lg"></i>
                        </button>
                        <button @click="confirmDeleteDay(day.dateKey)" class="w-8 h-8 md:w-10 md:h-10 rounded-full border border-outline-variant flex items-center justify-center hover:bg-red-500 hover:text-white transition-all text-on-surface-variant bg-surface-container-low md:bg-transparent">
