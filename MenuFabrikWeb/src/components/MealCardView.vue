...
<template>
    <div 
        class="meal-card flex flex-col gap-3 p-4 rounded-xl border transition-all overflow-hidden relative"
        :class="[categoryColorClass, {'pb-3': hasRecipe}]"
    ...
    >
        ...
        <div class="flex-1 flex flex-col gap-2" :class="{'pl-3': hasRecipe}">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center ml-1">
                        <Button v-if="hasRecipe" icon="pi pi-refresh" text rounded severity="secondary" size="small" style="width: 2rem; height: 2rem; padding: 0;" @click.stop="emit('generate')" aria-label="Alternative" title="Proposer une alternative" />
            ...
            <div v-if="hasRecipe" class="flex flex-col gap-1 mt-1 cursor-pointer group" @click="emit('click')" title="Voir les détails">
                <h3 class="text-lg font-semibold group-hover:text-primary-600 text-surface-900 dark:text-surface-0 leading-tight transition-colors">
                    {{ meal.recipe?.name }}
                </h3>
            ...
            <div v-else-if="isPlanned" class="flex flex-col items-center justify-center p-3 border-2 border-dashed border-surface-200 dark:border-surface-700 rounded-lg bg-surface-50/50 dark:bg-surface-800/50 mt-1 hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors cursor-pointer" @click.stop="emit('generate')">
                <i class="pi pi-sparkles text-surface-400 dark:text-surface-500 text-xl mb-1 mt-1"></i>
                <span class="text-sm font-medium text-surface-500 dark:text-surface-400 mb-1">Générer {{ meal.type.toLowerCase() }}</span>
            </div>
        </div>
    </div>
</template>
