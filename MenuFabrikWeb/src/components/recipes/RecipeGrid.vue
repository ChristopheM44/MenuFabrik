...
const gridClass = computed(() =>
    props.gridCols ?? 'grid-cols-1 md:grid-cols-2'
);
...
<template>
    ...
    <!-- Empty State -->
    <div v-else-if="recipes.length === 0" class="text-center p-12 text-on-surface-variant bg-surface-container-low rounded-3xl border border-dashed border-outline-variant">
        <i class="pi pi-inbox text-5xl mb-4 text-on-surface-variant"></i>
    ...
    <div v-else class="grid gap-4 md:gap-5" :class="gridClass">
        <div 
            ...
            :key="recipe.id"
            class="group bg-surface-container-lowest rounded-2xl overflow-hidden transition-all duration-300 shadow-sm hover:shadow-xl flex flex-row items-stretch h-32 md:h-36 border border-outline-variant"
            :class="{
        ...
        >
            <!-- Image Section -->
            <div 
                class="relative w-32 md:w-36 flex-shrink-0 overflow-hidden bg-surface-container-low flex items-center justify-center border-r border-outline-variant"
            ...
            <div class="p-3 md:p-4 flex flex-col flex-1 min-w-0 justify-between">
                <div>
                    <div class="flex items-center gap-2 mb-1.5 text-on-surface-variant text-[10px] font-bold uppercase tracking-widest">
                    ...
                    <h3 class="font-bold text-on-surface text-[14px] md:text-[15px] leading-tight line-clamp-2">{{ recipe.name }}</h3>
                </div>
                ...
                <div class="flex items-center justify-between mt-2 pt-2 border-t border-outline-variant border-dashed">
                    ...
                    <div v-else class="flex items-center gap-1">
                        ...
                        <i v-else class="pi pi-chevron-right text-on-surface-variant text-base"></i>
                    </div>
                </div>
            </div>
        </div>
